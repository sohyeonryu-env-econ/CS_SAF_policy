# extended_grid.jl  (endo non-soy version)
cd(@__DIR__)
println("Working directory: ", pwd())

include(joinpath(@__DIR__, "model_endo_nonsoy.jl"))
include(joinpath(@__DIR__, "analysis_endo_nonsoy.jl"))
...
begin
    ...
    # add vertical lines to the plot (3B, 5B)
...
function plot_mac_comparison_simple(results_extended_analysis, mac_extended;
    vlines_abatement=nothing, y_max=500.0, y_min=-250.0, fig_size=(1600, 1200))
...
mac=plot_mac_comparison_simple(results_extended_analysis, mac_extended; vlines_abatement=vlines_abatement, y_max=500.0, y_min=-250.0)
savefig(mac, joinpath(FIGURE_DIR, "nonsoy_endo_MAC.png"))
...
# ── Welfare summary (simple line version) ─────────────────────────────────────
function plot_welfare_summary_line_by_policy(results_extended_analysis; vlines=nothing)
    welfare_summary = results_extended_analysis.welfare_summary
...
# ── Non-soy quantity (endo-specific) ─────────────────────────────────────────
