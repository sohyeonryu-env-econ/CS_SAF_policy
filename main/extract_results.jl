...
@load joinpath(OUTPUT_DIR, "results_target.jld2") results_target policy_configs_target equivalent_policies welfare_target target_saf target_emissions
...
function get_solution(scenario, group)
    if group in (:observed, :validation)
        return nothing
    elseif group == :base
        return results_base[scenario]
    else
        return results_target[scenario]
    end
end
...
for (scenario, group) in scenario_order
    ...
    elseif group == :base
        ...
        config = policy_configs_target[scenario]
...
for (scenario, group) in scenario_order
    if group in (:observed, :validation, :base)
        ...
        target_em_dict[(scenario, group)] = target_emissions
        target_red_dict[(scenario, group)] = results_target[scenario].target_emissions
    end
end
